    ...
    class Sesija
    {
        ...
        private static ISessionFactory NapraviSesiju()
        {
            try
            {
                ...
                //.ConnectionString(c => c.FromConnectionStringWithKey(KonekcioniString));

                return Fluently.Configure().Database(MySQLConfiguration.Standard
                .ConnectionString(c => c.FromConnectionStringWithKey(KonekcioniString))).Mappings(m => m.FluentMappings.AddFromAssemblyOf<KorisnikMapiranje>()).BuildSessionFactory();
